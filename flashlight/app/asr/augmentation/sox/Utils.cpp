...
// void validate_input_tensor(const std::vector<float> tensor) {
...
// std::vector<float> unnormalize_wav(const std::vector<float> input_tensor) {
...
//       /*length=*/static_cast<unsigned long long>(waveform->numel())};
// }
...
unsigned long long read_fileobj(py::object* fileobj, const unsigned long long size, char* buffer) {
  unsigned long long num_read = 0;
  while (num_read < size) {
...
TORCH_LIBRARY_FRAGMENT(fl, m) {
  m.def("fl::sox_utils_set_seed", &fl::sox_utils::set_seed);
  m.def(
      "fl::sox_utils_set_verbosity",
      &fl::sox_utils::set_verbosity);
  m.def(
      "fl::sox_utils_set_use_threads",
      &fl::sox_utils::set_use_threads);
  m.def(
      "fl::sox_utils_set_buffer_size",
      &fl::sox_utils::set_buffer_size);
  m.def(
      "fl::sox_utils_list_effects",
      &fl::sox_utils::list_effects);
  m.def(
      "fl::sox_utils_list_read_formats",
      &fl::sox_utils::list_read_formats);
  m.def(
      "fl::sox_utils_list_write_formats",
      &fl::sox_utils::list_write_formats);
}
