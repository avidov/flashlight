...
Variable batchnorm(
  ...
  {
    MiOpenDevicePtr inRaw(input.array());
    MiOpenDevicePtr outRaw(output);
    MiOpenDevicePtr wtRaw(weightArray);
    MiOpenDevicePtr bsRaw(biasArray);
    MiOpenDevicePtr runMeanRaw(runningMean.array());
    MiOpenDevicePtr runVarRaw(runningVar.array());
    ...
    if (train) {
      ...
      MiOpenDevicePtr saveMeanRaw(saveMean);
      MiOpenDevicePtr saveVarRaw(saveVar);
  ...
  auto gradFunc =
      [train, saveMean, saveVar, mode, inDescDims, wtDescDims, epsilon](
        ...
        {
          MiOpenDevicePtr iRaw(inArray);
          MiOpenDevicePtr wRaw(wt.array());

          MiOpenDevicePtr gradInRaw(gradIn);
          MiOpenDevicePtr gradWtRaw(gradWt);
          MiOpenDevicePtr gradBsRaw(gradBs);

          MiOpenDevicePtr gradOpRaw(gradOutputArray);
