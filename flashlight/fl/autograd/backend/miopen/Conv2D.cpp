...
Variable conv2d(
  ...
  {
    // FL_LOG(fl::INFO) << "output.get()=" << output.get();
    MiOpenDevicePtr inPtr(input.array());
    MiOpenDevicePtr wtPtr(weights.array());
    MiOpenDevicePtr outPtr(output);
    // FL_LOG(fl::INFO) << "output.get()=" << output.get() << " outPtr.arr_=" << outPtr.arr_;
  ...
  auto gradFunc = [sx, sy, px, py, dx, dy, hasBias, groups](
    ...
    if (hasBias && inputs.size() > 2 && inputs[2].isCalcGrad()) {
      ...
      auto convolutionBackwardBias = [&bias, &hndl, oneg, zerog](
        ...
        MiOpenDevicePtr gradResultPtr(gradOutput);

        auto gradBias =
            Variable(af::array(bsArray.dims(), bsArray.type()), false);
        {
          MiOpenDevicePtr gradBiasPtr(gradBias.array());
    ...
    auto convolutionBackwardData = [&hndl, &in, oneg, zerog, dx, dy, &wt](
      ...
      if (in.isCalcGrad()) {
        ...
        {
          MiOpenDevicePtr gradInputPtr(gradInput.array());
          MiOpenDevicePtr gradResultPtr(gradOutputArray);
          MiOpenDevicePtr wsPtr(ws);
    ...
    auto convolutionBackwardFilter = [&hndl, &wt, oneg, zerog](
      ...
      if (wt.isCalcGrad()) {
        ...
        {
          MiOpenDevicePtr gradWeightPtr(gradWeight.array());
          MiOpenDevicePtr gradResultPtr(gradOutputArray);
          MiOpenDevicePtr wsPtr(ws);
